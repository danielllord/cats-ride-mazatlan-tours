...
function Index() {
  const stops = [
    ...
    { title: "Historic Center & Beautiful Cathedral", src: triptychImage, position: "left" },
    { title: "Municipal Market", src: triptychImage, position: "center" },
    { title: "Relax at a Stunning Beach", note: "Pool & resort access where available", src: triptychImage, position: "right" },
  ];
  return <main id="home"><Header />
    ...
    <section id="stops" className="bg-secondary py-16 md:py-20"><div className="mx-auto max-w-7xl px-5 md:px-8"><div className="text-center"><h2 className="text-4xl text-primary md:text-5xl">Popular Stops on Our Tour</h2><p className="mt-2 text-xs font-extrabold uppercase tracking-[.2em] text-ocean">A flexible itinerary designed around you</p></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{stops.map((stop) => <article key={stop.title} className="overflow-hidden rounded-lg bg-card shadow-lg"><div className="aspect-[4/3] overflow-hidden"><img src={stop.src} alt={`${stop.title} in Mazatlán`} width="1024" height="768" loading="lazy" className={`h-full w-full object-cover transition-transform duration-500 hover:scale-105 ${stop.position === "left" ? "object-left" : stop.position === "right" ? "object-right" : "object-center"}`} /></div><div className="min-h-24 p-5 text-center"><h3 className="text-xl text-primary">{stop.title}</h3>{stop.note && <p className="mt-1 text-sm text-muted-foreground">{stop.note}</p>}</div></article>)}</div></div></section>

    <section className="grid md:grid-cols-2"><div className="ocean-wash flex min-h-96 items-center px-6 py-16 text-primary-foreground md:px-[10vw]"><div><Sparkles className="mb-5 h-10 w-10 text-sunshine" /><h2 className="text-4xl md:text-5xl">Custom Tours for<br />Your Interests</h2><p className="mt-6 max-w-xl text-lg leading-8 text-primary-foreground/85">Enjoy time for lunch, optional tequila tasting, colorful Mazatlán stops, beaches, sightseeing, shopping, and plenty of opportunities for photos. Your itinerary can be customized around your interests and available time.</p></div></div><div className="sunset-wash flex min-h-96 items-center px-6 py-16 md:px-[10vw]"><div><GlassWater className="mb-5 h-10 w-10 text-primary" /><h2 className="text-4xl text-primary md:text-5xl">Sip, Relax & Enjoy</h2><p className="mt-6 max-w-xl text-lg leading-8 text-primary/80">We also offer drinks including water, sodas, Pacifico and Corona beer.</p></div></div></section>
